...
$(document).ready(async function () {
    ...
    if (parseInt(CotizacionData) > 0) {
        await cargarDatosCotizacion(CotizacionData);
    } else {
...
async function cargarDatosCotizacion(id) {
    $("#tituloCotizacion").text(`Editar Cotizacion ${id}`);
    const datosCotizacion = await ObtenerDatosCotizacion(id);
    await configurarDataTableProductos(datosCotizacion.CotizacionDetalle);
    await configurarDataTableInsumos(datosCotizacion.CotizacionDetalleProceso);
    await insertarDatosCotizacion(datosCotizacion);


}


async function insertarDatosCotizacion(datosCotizacion) {

    document.getElementById("IdCotizacion").value = datosCotizacion.Cotizacion.Id;
    document.getElementById("Clientes").value = parseInt(datosCotizacion.Cotizacion.IdCliente);
    document.getElementById("Telefono").value = datosCotizacion.Cotizacion.Telefono;
    document.getElementById("Fecha").value = moment(datosCotizacion.Cotizacion.Fecha, 'YYYY-MM-DD').format('YYYY-MM-DD');
    document.getElementById("Formasdepago").value = parseInt(datosCotizacion.Cotizacion.IdFormaPago);
    document.getElementById("ImporteTotal").value = datosCotizacion.Cotizacion.ImporteTotal;
    document.getElementById("PorcDesc").value = datosCotizacion.Cotizacion.PorcDescuento;
    document.getElementById("SubTotal").value = datosCotizacion.Cotizacion.SubTotal;
    document.getElementById("ImporteAbonado").value = datosCotizacion.Cotizacion.ImporteAbonado;
    document.getElementById("Saldo").value = datosCotizacion.Cotizacion.Saldo;
    document.getElementById("Comentarios").value = datosCotizacion.Cotizacion.Comentarios ?? "";
...
$('#ProductoModalCantidad').on('keyup', function () {
    ...
    if (filasSeleccionadas.length > 0) {
        ...
        $('#grd_Insumos_Modal tbody tr').each(function () {
            ...
            const idCotizacion = $("#IdCotizacion").val();
    ...
    if (gridProductos == null) {
        gridProductos = $('#grd_Productos').DataTable({
            ...
            initComplete: async function () {
                ...
                $('body').on('mouseenter', '#grd_Cotizaciones .fa-map-marker', function () {
                    $(this).css('cursor', 'pointer');
                });
...
function configurarOpcionesColumnasProductos() {
    ...
    const storageKey = `Cotizaciones_Productos_Columnas`; // Clave única para esta pantalla
...
function configurarOpcionesColumnasInsumos() {
    ...
    const storageKey = `Cotizaciones_Insumos_Columnas`; // Clave única para esta pantalla
...
function calcularDatosCotizacion() {
    let importeTotal = 0;
...
document.getElementById("PorcDesc").addEventListener("blur", calcularDatosCotizacion);
...
document.getElementById("ImporteAbonado").addEventListener("blur", function () {
    ...
    calcularDatosCotizacion(); // Recalcular los datos
});


async function ObtenerDatosCotizacion(id) {
    const url = `/Cotizaciones/ObtenerDatosCotizacion?idCotizacion=${id}`;
    const response = await fetch(url);
    const data = await response.json();
    return data;
}
...
async function guardarCambios(redirecciona = true) {
    ...
    try {
        ...
        if (!isValidCotizacion()) return false;
        ...
        // --- Payload ---
        const nuevoModelo = {
            "Id": idCotizacion !== "" ? parseInt(idCotizacion) : 0,
        ...
        // --- Endpoint ---
        const url = idCotizacion === "" ? "/Cotizaciones/Insertar" : "/Cotizaciones/Actualizar";
        const method = idCotizacion === "" ? "POST" : "PUT";
        ...
        if (!ok) {
            const msg = serverMessage || 'No se pudo guardar la cotizacion.';
            if (typeof errorModal === 'function') errorModal(msg);
            else alert(msg);
            return false;
        }
        ...
        // --- Éxito ---
        const mensajeOk = serverMessage || (idCotizacion === "" ? "Cotizacion registrado correctamente" : "Cotizacion modificado correctamente");
        ...
        const msg = (error && error.message) ? error.message : 'Ocurrió un error al guardar la cotizacion.';
        if (typeof errorModal === 'function') errorModal(msg);
        else alert(msg);
        return false;
    }
...
function isValidCotizacion() {
    ...
    if (IdCliente == -1) {
        errorModal("Para crear un Cotizacion, debes seleccionar un cliente.");
        return false;
    }
    ...
    if (cantidadFilas <= 0) {
        if (IdCotizacion == "") {
            errorModal('No puedes crear un Cotizacion sin productos.')
        } else {
            errorModal('No puedes modificar un Cotizacion sin productos.')
        }
        return false;
    }
...
function generarDatosCotizacionPDF() {
    ...
    factura = generarCotizacionPDF(datos);
    facturaCliente = cliente;
    descargarCotizacionPDF(datos, factura);
}

async function generarCotizacionPDF(datos) {
    ...
    // Datos cliente
    doc.setFontSize(8);
    doc.text(`Nombre: ${datos.Cotizacion.Cliente}`, 150, 37);
    doc.text(`Teléfono: ${datos.Cotizacion.Telefono}`, 150, 41);
    doc.text(`Fecha: ${moment(datos.Cotizacion.Fecha, "YYYY-MM-DD").format("DD/MM/YYYY")}`, 150, 45);
...
async function descargarCotizacionPDF(datos) {
    const doc = await generarCotizacionPDF(datos);
...
function descargarRemitoPDF(datos, facturaPDF) {
    ...
    if (datos.Cotizacion.IdCotizacion == "") {
        msjCotizacion = ""
    } else {
        msjCotizacion = `Nº ${datos.Cotizacion.IdCotizacion} `
    }
...
async function registrarComoPedido() {
    try {
        ...
        // Validaciones mínimas (coherentes con tus reglas actuales)
        if (!idCliente || idCliente === '-1') {
            errorModal("Para crear el Pedido, debes seleccionar un cliente.");
            return;
        }
        if (cantProductos <= 0) {
            errorModal("No puedes convertir una cotización sin productos.");
            return;
        }
        ...
        // [HttpPost] Cotizaciones/RegistrarComoPedido?id=##
        ...
        if (!resp.ok) {
            ...
            throw new Error(msg || 'Error al registrar la Cotización como Pedido.');
        }

        const data = await resp.json();
        if (data?.ok && data?.idPedido > 0) {
            exitoModal('Cotización registrada exitosamente como Pedido.');
            // Redirigir al pedido creado (tu patrón actual de edición)
            setTimeout(() => {
                window.location.href = `/Pedidos/NuevoModif/${data.idPedido}`;
            }, 900);
        } else {
        ...
        console.error(err);
        errorModal(err.message || 'Error inesperado al convertir en Pedido.');
    } finally {
        // Rehabilitar el botón
        $('#btnRegistrarPedido').prop('disabled', false).html('<i class="fa fa-shopping-cart me-1"></i> Registrar como Pedido');
    }
}